import { Authorization, Token, UserInfo } from "@/constants/authoriztions";
import { getSearchValue } from "../utils/commonUtils";
...
const storage = {
  ...
  removeAll: () => {
    KeySet.forEach((x) => {
      // khong xoa khi key = userLanguage
      if (x !== "userLanguage") {
...
export function redirectToLogin() {
  window.location.href = location.origin + `/sign-in`;
}
