...
export function SubjectDetailPanel({
  ...
  if (isPending) {
    return (
      <div className="flex items-center gap-2 text-sm text-slate-500">
        ...
        {t("corpus.detail.loadingSubject")}
      </div>
    );
  }
  ...
  if (envelope && envelope.code !== 200) {
    return (
      <p className="text-sm text-rose-600">
        {envelope.msg || t("corpus.detail.loadSubjectFailed")}
      </p>
    );
  }
  ...
  return (
    <div>
      ...
      <dl className="mt-4">
        <DetailRow icon={FileText} label={t("corpus.detail.topicLabel")} value={detail?.topic_title} />
        <DetailRow
          icon={ListOrdered}
          label={t("corpus.detail.subjectNumber")}
          value={detail?.subject_number}
        />
        <DetailRow
          icon={FileText}
          label={t("corpus.detail.sourceUrl")}
      ...
      <div className="mt-6 border-t border-slate-100 pt-4">
        <div className="mb-3 flex items-center justify-between gap-2">
          <h5 className="text-sm font-bold text-slate-800">
            {t("corpus.detail.articleList")}
          </h5>
          {!articlesLoading && articles.length > 0 && (
            <span className="text-xs font-medium text-slate-500">
              {t("corpus.detail.itemsCount", { count: articles.length })}
            </span>
          )}
        </div>
        ...
        {articlesLoading ? (
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <Loader2 className="h-4 w-4 animate-spin" />
            {t("corpus.detail.loadingArticles")}
          </div>
        ) : articlesEnvelope && articlesEnvelope.code !== 200 ? (
          <p className="text-sm text-rose-600">
            {articlesEnvelope.msg || t("corpus.detail.loadArticlesFailed")}
          </p>
        ) : articles.length === 0 ? (
          <p className="text-sm text-slate-500">{t("corpus.detail.noArticles")}</p>
        ) : (
